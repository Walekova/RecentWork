...
# MAGIC %md ### Run the next four cells to create your directory in dbfs
...
# MAGIC > __a)__ is a cluster computing platform that extends MapReduce model to efficiently support more types of computations, including interactive queries and stream processing.
# MAGIC 
# MAGIC > __b)__ Data is immutable, it has higher order functions, and it is lazy. However Spark does allow side effects.
# MAGIC 
# MAGIC > __c)__
# MAGIC 
# MAGIC __What is DAG__  
# MAGIC DAGScheduler is the scheduling layer of Apache Spark that implements stage-oriented scheduling. It transforms a logical execution plan to a physical execution plan using stages.
# MAGIC 
# MAGIC - Directed – Means which is directly connected from one node to another. This creates a sequence
# MAGIC - Acyclic – Defines that there is no cycle or loop available. Once a transformation takes place it cannot returns to its earlier position.
# MAGIC - Graph – From graph theory, it is a combination of vertices and edges. 
# MAGIC 
# MAGIC __How do they relate to the difference between an 'action' and a 'transformation'?__
# MAGIC Stages are created based on the transformations. The narrow transformations will be grouped (pipe-lined) together into a single stage. Shuffle Operation or wide transformation define the boundary of 2 stages.Stages are separated by 2 shuffle operations.
# MAGIC 
# MAGIC __Why is it useful to pay attention to the DAG that underlies your Spark implementation?__
# MAGIC The stages that are not interdependent may be submitted to the cluster for execution in parallel: this maximizes the parallelization capability on the cluster. So if operations in our dataflow can happen simultaneously we will see multiple stages launched.
# MAGIC 
# MAGIC We optimize the DAG in Apache Spark by rearranging and combining operators wherever possible. For, example if we submit a spark job which has a map() operation followed by a filter operation. The DAG Optimizer will rearrange the order of these operators since filtering will reduce the number of records to undergo map operation.
# MAGIC 
# MAGIC > __d)__ 
# MAGIC 
# MAGIC __Give a specific example of when we would want to cache() an RDD__
# MAGIC When we would want to use the same RDD multiple times. Example KMEANS.
# MAGIC 
# MAGIC __Explain why.__
# MAGIC This is because during first iterations data and code is sent to slaves. In Mapreduce every iteration this needs to take place. This is a costly procedure. If we want to ietrate if the RDD remains in memory / cached then repeating the operations will be faster.
...
# MAGIC 
# MAGIC * __c) short response:__ Calculate the cosine similarity for each pair of documents in our toy corpus. Please use markdown and $\LaTeX$ to show your calculations.  
# MAGIC 
# MAGIC * __d) short response:__ According to your calculations in `part c` which pair of documents are most similar in meaning? Does this match your expectation from reading the documents? If not, speculate about why we might have gotten this result.
# MAGIC 
...
# MAGIC > __a)__ cosine similarity
# MAGIC 
# MAGIC __numerator__ measures the number of dimensions on which both vectors agree
# MAGIC 
# MAGIC __denominator__ is related to the size of the sets that each vector represents.
# MAGIC 
# MAGIC > __b)__ 
# MAGIC 
# MAGIC __Jaccard, Overlap and Dice metrics are similar/different to the calculation for cosine similarity.__
# MAGIC 
# MAGIC _Jaccard_
# MAGIC - in cosine similarity, the number of common attributes is divided by the size of the sets that each vector represents. Whereas in Jaccard Similarity, the number of common attributes is divided by the number of attributes that exists in at least one of the two objects. 
# MAGIC - Jaccard Coefficient can be used as a dissimilarity or distance measure, whereas the cosine similarity has no such constructs.
# MAGIC - Jaccard similarity takes only unique set of words for each sentence / document while cosine similarity takes total length of the vectors. This means that if you repeat a word in a sentence several times, cosine similarity changes but Jaccard similarity does not.
# MAGIC 
# MAGIC _Overlap_
# MAGIC - in cosine similarity, the number of common attributes is divided by the size of the sets that each vector represents. Whereas in Overlap Similarity, the number of common attributes is divided by the smaller of the size of the two sets.
# MAGIC 
# MAGIC _Dice_
# MAGIC - Dice simply checks if a word from a vocabulary is present in both documents and not for the frequency of the occurences. 
# MAGIC - Cosine includes frequency by using the scalar product, and instead of doing a check if a word is present in both sets, it multiplies the frequencies of words in both documents
# MAGIC 
# MAGIC __When would these metrics lead to different similarity rankings for a set of documents?__
# MAGIC _Jaccard_
# MAGIC - Jaccard similarity is not able to capture semantic/lexical similarity of two sentences.
# MAGIC - Jaccard similarity is good for cases where duplication does not matter, cosine similarity is good for cases where duplication matters while analyzing text similarity. 
# MAGIC - as the size of the document increases, the number of common words tend to increase even if the documents talk about different topics.
# MAGIC 
# MAGIC _Overlap_
# MAGIC - Can use the Overlap Similarity algorithm to work out which things are subsets of others.
# MAGIC 
# MAGIC _Dice_
# MAGIC - Sørensen distance retains sensitivity in more heterogeneous data sets and gives less weight to outliers
# MAGIC 
# MAGIC > __c)__ LATEX calculations  
# MAGIC 
# MAGIC - A[1,1,0,1,0,1,1]
# MAGIC - B[1,0,0,1,1,1,1]
# MAGIC - C[0,1,1,1,0,0,0]
# MAGIC 
# MAGIC - AB 
# MAGIC $$AB = \frac{1\times1 + 1\times0 + 0\times0 + 1\times1 + 0\times1 + 1\times1 + 1\times1}{\sqrt{1^2 + 1^2 + 0^2 + 1^2 + 0^2 + 1^2 + 1^2} \times \sqrt{1^2 + 0^2 + 0^2 + 1^2 + 1^2 + 1^2 + 1^2} } = \frac{4}{\sqrt{5} \times \sqrt{5}} = \frac{4}{5} = 0.8$$
# MAGIC - CB
# MAGIC $$CB = \frac{0\times1 + 1\times0 + 1\times0 + 1\times1 + 0\times1 + 0\times1 + 0\times1}{\sqrt{0^2 + 1^2 + 1^2 + 1^2 + 0^2 + 0^2 + 0^2} \times \sqrt{1^2 + 0^2 + 0^2 + 1^2 + 1^2 + 1^2 + 1^2} } = \frac{1}{\sqrt{3} \times \sqrt{5}} = 0.258$$
# MAGIC - AC
# MAGIC $$AC = \frac{1\times0 + 1\times1 + 0\times1 + 1\times1 + 0\times0 + 1\times0 + 1\times0}{\sqrt{1^2 + 1^2 + 0^2 + 1^2 + 0^2 + 1^2 + 1^2} \times \sqrt{0^2 + 1^2 + 1^2 + 1^2 + 0^2 + 0^2 + 0^2} } = \frac{2}{\sqrt{5} \times \sqrt{3}} = 0.516$$
# MAGIC 
# MAGIC > __d)__ 
# MAGIC 
# MAGIC __most similar in meaning__
# MAGIC A and B
# MAGIC __Does this match your expectation from reading the documents?__
# MAGIC No it does not match the expectation. A and C are more similar. C does not have any of the stop words. Based on cosine similarity the stop words are common for both A and B and therefore the cosine similarity is higher for these.
# MAGIC 
# MAGIC > __e)__ ignoring 'the', 'of', 'a'
# MAGIC - D[1,0,1,0]
# MAGIC - E[0,0,1,1]
# MAGIC - F[1,1,1,0]
# MAGIC 
# MAGIC - DE \
# MAGIC $$DE = \frac{1\times0 + 0\times0 + 1\times1 + 0\times1 }{\sqrt{1^2 + 0^2 + 1^2 + 0^2} \times \sqrt{0^2 + 0^2 + 1^2 + 1^2} } = \frac{1}{\sqrt{2} \times \sqrt{2}} = \frac{1}{2} = 0.2$$
# MAGIC - FE
# MAGIC $$FE = \frac{1\times0 + 1\times0 + 1\times1 + 0\times1 }{\sqrt{1^2 + 1^2 + 1^2 + 0^2 } \times \sqrt{0^2 + 0^2 + 1^2 + 1^2} } = \frac{1}{\sqrt{3} \times \sqrt{2}} = 0.408$$
# MAGIC - DF
# MAGIC $$DF = \frac{1\times1 + 0\times1 + 1\times1 + 0\times0 }{\sqrt{1^2 + 0^2 + 1^2 + 0^2} \times \sqrt{1^2 + 1^2 + 1^2 + 0^2} } = \frac{2}{\sqrt{2} \times \sqrt{3}} = 0.816$$
# MAGIC 
# MAGIC __How do results change?__
# MAGIC The outcome is in line with expectations. By removing the stop words the cosine similarity takes into account only the words that have the true meanimg
...
# MAGIC > __a)__ 
# MAGIC 
# MAGIC - bigram with words next to each other $$N_{grams} = X - (N - 1) = 6-1 = 5$$ where X - number of words in text, N - size of N-gram.
# MAGIC - co-occurence in a sentence. $$N_{grams} = C(6,2) = \frac{6!}{(6-2)!2!} = \frac{30}{2} = 15$$
# MAGIC 
# MAGIC 
# MAGIC > __c)__ 
# MAGIC 
# MAGIC __For our 6 word vocabulary how many entries are in this matrix?__ 36
# MAGIC 
# MAGIC __How many entries are zeros?__ 36 - 8 = 28
# MAGIC 
# MAGIC > __e)__ 
# MAGIC 
# MAGIC - __Which pairs of words have the highest 'similarity' scores?__
# MAGIC best and worst / wisdom & foolishness
# MAGIC - __Are these words 'synonyms' in the traditional sense?__
# MAGIC no actually these are antonyms
# MAGIC - __In what sense are their meanings 'similar'?__
# MAGIC they are used in similar way, they are the opposite sides of a 'scale'
# MAGIC - __Explain how our results are contingent on the input text__
# MAGIC the words occure in the exact same sentence construct with the same words. In this example it is difficult to determine whether it is an antonym or a synonym as there is nothing to diferentiate the two.
# MAGIC - __What would change if we had a much larger corpus?__
# MAGIC i would expect that the words will co-occur with additional descriptive adjectives which is going to result in improved distinction among synonyms and antonyms
...
def getStripes(corpus, vocab, stripes, ngram):
  """ The function to create stripes, a word co-ccurrence dictionary
  
  INPUT
  corpus - STRING; a text to be examined
  vocab - LIST; list of unique non stop words from the corpus
  stripes - DICTIONARY;
  ngram - INTEGER; how many adjacent words to search. i.e. '2' will look at two to the left and right, in total four words
  
  OUTPUT
  stripes - DICTIONARY; word co-ccurrence dictionary
  """
  
  words = re.findall(r'[a-z]+', corpus.lower())
  
  # get basic positions of ngrams
  ngramList = []
  ...
  for i in range(len(words)):
    # is the word in vocab / i.e. not a stop word
    if words[i] in vocab:
      # based on ngram size check if words are in vocab
      for j in ngramList:
        if i+j in range(len(words)):
          if words[i+j] in vocab:
            # update co-occurrence dictionary / stripes
            if words[i+j] not in stripes[words[i]]:
              stripes[words[i]].append(words[i+j])
  
  return stripes

getStripes(corpus, vocab, stripes, 2)
...
for term1, term2 in itertools.combinations(vocab, 2):
    ...
    ############# YOUR CODE HERE #################
    csim = None
    numerator = v1.multiply(v2).sum()
    term1Length = float(v1.multiply(v1).sum())**(0.5)
    term2Length = float(v2.multiply(v2).sum())**(0.5)
    csim = numerator / (term1Length * term2Length)
...
# MAGIC > __a)__ 
# MAGIC 
# MAGIC __STEP 1__ memory - stdin - sentence/ text
# MAGIC - map - read stdin line/ word by work, create output 5 entries key/word1/word2/word3/word4
# MAGIC - shuffle - shuffle by key
# MAGIC - reduce - create dictionary and unique vocab
# MAGIC 
# MAGIC __STEP 2__ read dictionary into memory. read 
# MAGIC - map - for each entry in vocab, provide entry into matrix key - value [word, 010001] that represents full vocab and 1 and 0 per dictionary entry
# MAGIC - reduce - /bin/cat
# MAGIC 
# MAGIC __STEP 3__ read the matrix into memory
# MAGIC - map for term1, term2 in itertools.combinations(vocab, 2) calculate cosine similarity
# MAGIC - reduce - /bin/cat
# MAGIC 
# MAGIC > __b)__ Explain two ways that using these data structures are more efficient than 1-hot encoded vectors when it comes to distributed similarity calculations 
# MAGIC memory constraints / shuffle / network / parallelization
# MAGIC 
# MAGIC 1. PAIRS - a lot of pairs to sort and shuffle across the network.
# MAGIC 2. STRIPES -  less to shuffle across the network than when we use PAIRS but associative arrays cannot be larger than what would fit into memory
# MAGIC 
# MAGIC > __c)__ _read provided code before answering d-f_ 
# MAGIC 
# MAGIC > __d)__ Define each of these terms ('postings' and 'inverted index') 
# MAGIC 
# MAGIC __postings__ a posting is a tuple of document id and payload information, this can be either frequency a number of times a term occurs. in our case the information is the number of words in the respective document.
# MAGIC 
# MAGIC __inverted index__ is an index of terms that have associated list of documents i.e posting lists
# MAGIC 
# MAGIC > __e)__ The third mapper, makeCompositeKeys, loops over the inverted index to emit 'pairs' of what? Explain what information is included in the composite key created at this stage and why it makes sense to synchronize around that information in the context of performing document similarity calculations. In addition to the information included in these new keys, what other piece of information will we need to compute Jaccard or Cosine similarity? 
# MAGIC 
# MAGIC __emit 'pairs' of what?__ documents
# MAGIC 
# MAGIC __what information is included in the composite key__ pair of two documents
# MAGIC 
# MAGIC __why it makes sense to synchronize around that information in the context of performing document similarity calculations__ because it defines the pair of documents ffor which we want to calculate the similarity
# MAGIC 
# MAGIC __what other piece of information will we need to compute Jaccard or Cosine similarity__ dot product, which is calculated in the next ReduceByKey
# MAGIC 
# MAGIC 
# MAGIC > __f)__ Out of all the Spark transformations we make in this analysis, which are 'wide' transformations and which are 'narrow' transformations. Explain.
# MAGIC 
# MAGIC - __wide__ reduceByKey
# MAGIC - __narrow__ Map, flatMap
# MAGIC 
# MAGIC reduceByKey triggers a shuffle.
